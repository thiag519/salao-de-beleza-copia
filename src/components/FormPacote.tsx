...
import { PacotesServicosType } from "@/types/pacotesTypes/PacotesServicosType"
import { Modal } from "./Modal"
import { useActive } from "@/hooks/useActive"
import Link from "next/link"

type Props = {
  form: PacotesServicosType[]
  phoneNumber: string
}

export const FormPacote = ({form, phoneNumber}:Props) => {
  const [data, setData] = useState<FormDataEntryValue[]>();
  const {active, toggleActive} = useActive();
  const [name, setName] = useState('')

  const handlerFormData = (e: FormEvent<HTMLFormElement>) => { 
    e.preventDefault()
    ...
    if(query.length > 0) {
      ...
      alert('Escolha um serviço, ou volte no botão na parte superior a direita, e continuar sua busca')
    }
  }
  ...
  if(data){ 
    for (let i = 0; i < data.length; i++) {
      ...
      if(elementString.includes('R$') ) {
        ...
        prices = elementString.replace(/\D+/g, '').slice(0, -2);
        console.log(prices)
        prices &&
        valores.push(parseFloat(prices))
      };
  ...
  return (
    <div className="flex items-center justify-center w-full ">
      <form
        className="w-full h-full md:w-[700px] flex flex-col justify-center 
        p-2 md:p-5 rounded-xl shadow-2xl"
        onSubmit={handlerFormData}
      >
        <h1 className="w-full hidden sm:block py-1 md:py-5 text-center">
          Escolha e agende já
        </h1>
        <div
        ...
        {form.map((item, index) => (
          <div
            key={index}
            className={` flex justify-between my-1 hover:bg-[var(--color-highlight)]/30 border-b-2 
          ...
          >
            ...
            {!item.valor && item.nome === "Total" && item.valortotal !== 0 && (
              <p className="hidden">{(valorT = item.valortotal)}</p>
            )}
            {!item.valor && item.nome !== "Total" && (
              <input
                readOnly
                checked
                value={` ${item.nome}\n`}
            ...
            <label
              className="text-[13px] sm:text-[14px] md:text-[15px] lg:text-[17px] w-full text-end"
              htmlFor={`algo-${index.toString()}`}
            >
              {item.valor &&
              ...
              {item.nome === "Total" && item.valortotal !== 0 && (
